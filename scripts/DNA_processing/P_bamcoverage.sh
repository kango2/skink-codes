#!/bin/bash
#PBS -N P_bamcoverage
#PBS -l ncpus=8,mem=120gb,walltime=6:00:00,storage=gdata/if89+gdata/xl04,jobfs=120GB
#PBS -j oe
...
for i in $(ls *.bedgraph); do python ${workingdir}/scripts/process_bedgraph.py ${i} ${binsize} > ${workingdir}/bamcoverage_${directory_name}/fixed/${i/.bedgraph/}_fixed.bedgraph; done
