...
if [ "$FROM_TRIMMOMATIC" = "YES" ]; then
    while IFS=',' read -r col1 col2 col3 col4
    do
        echo -e "subread-align -t 0 -T ${PBS_NCPUS} --sortReadsByCoordinates -i ${workingdir}/subread_${directory_name}/index/genome -r ${workingdir}/trimmomatic_${directory_name}/${col2}_trimmed_1P.fq.gz -R ${workingdir}/trimmomatic_${directory_name}/${col2}_trimmed_2P.fq.gz -o ${workingdir}/subread_${directory_name}/mapping/${col2}_subread.bam"
    done < ${input_list} | parallel --jobs ${PBS_NCPUS} {}
else
    while IFS=',' read -r col1 col2 col3 col4
    do
        echo -e "subread-align -t 0 -T ${PBS_NCPUS} --sortReadsByCoordinates -i ${workingdir}/subread_${directory_name}/index/genome -r ${col3} -R ${col4} -o ${workingdir}/subread_${directory_name}/mapping/${col2}_subread.bam"
    done < ${input_list} | parallel --jobs ${PBS_NCPUS} {}
fi
